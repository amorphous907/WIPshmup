...
public class Player3 extends Player{
	...
	public Player3(Vector2 position, float width, float height, float hitX,
	...
	protected void HandleInput(World world) {
		if(world.keys[World.PLAYER_THREE_FIRE])
			gun.fire = true;
		else
			gun.fire = false;
		if(world.keys[World.PLAYER_THREE_UP])
			velocity.y = -Player.SPEED;
		if(world.keys[World.PLAYER_THREE_LEFT])
			velocity.x = -Player.SPEED;
		if(world.keys[World.PLAYER_THREE_DOWN])
			velocity.y = Player.SPEED;
		if(world.keys[World.PLAYER_THREE_RIGHT])
			velocity.x = Player.SPEED;
		
		if(!world.keys[World.PLAYER_THREE_UP] && !world.keys[World.PLAYER_THREE_DOWN])
			velocity.y = 0;
		if(!world.keys[World.PLAYER_THREE_LEFT] && !world.keys[World.PLAYER_THREE_RIGHT])
			velocity.x = 0;
	...
	protected void handleGamepad(World world){
		if(world.controllerButtons[GamepadHandler.PLAYER_THREE_A])
			gun.fire = true;
		else
			gun.fire = false;
		if(world.controllerPOV[GamepadHandler.PLAYER_THREE_POV_UP])
			velocity.y = -Player.SPEED;
		if(world.controllerPOV[GamepadHandler.PLAYER_THREE_POV_LEFT])
			velocity.x = -Player.SPEED;
		if(world.controllerPOV[GamepadHandler.PLAYER_THREE_POV_DOWN])
			velocity.y = Player.SPEED;
		if(world.controllerPOV[GamepadHandler.PLAYER_THREE_POV_RIGHT])
			velocity.x = Player.SPEED;
		
		if(!world.controllerPOV[GamepadHandler.PLAYER_THREE_POV_UP] && !world.controllerPOV[GamepadHandler.PLAYER_THREE_POV_DOWN])
			velocity.y = 0;
		if(!world.controllerPOV[GamepadHandler.PLAYER_THREE_POV_LEFT] && !world.controllerPOV[GamepadHandler.PLAYER_THREE_POV_RIGHT])
			velocity.x = 0;
		
		if(world.controllerAxis[GamepadHandler.PLAYER_THREE_LSTICK_Y] != 0)
			velocity.y = Player.SPEED*world.controllerAxis[GamepadHandler.PLAYER_THREE_LSTICK_Y];
		if(world.controllerAxis[GamepadHandler.PLAYER_THREE_LSTICK_X] != 0)
			velocity.x = Player.SPEED*world.controllerAxis[GamepadHandler.PLAYER_THREE_LSTICK_X];
	}
