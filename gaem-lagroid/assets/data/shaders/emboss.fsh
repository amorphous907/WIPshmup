...
void main() {
	...
	color.rgb += texture2D(u_sampler2D, texCoord - onePixel).rgb * contrast; // we need `.rgb` for older OpenGL versions (e.g. 2.0) which cannot yet do add a vec4 to a vec3 via += 
	color.rgb -= texture2D(u_sampler2D, texCoord + onePixel).rgb * contrast;
